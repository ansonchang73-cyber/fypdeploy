...
// Today's adherence reuses the same live (stream-backed) provider that
// already powers the caregiver's "Adherence Analytics" tab, rather than a
// second implementation of the same math against `medication_logs`.
...
/// Live adherence data for the *current calendar month*, for one patient —
/// e.g. "July 2026", not a rolling 30-day window. Real Riverpod provider
/// rather than a `Future` cached in `State`, specifically so pull-to-refresh
/// can force a genuinely fresh Firestore read: the caregiver shell keeps
/// every tab alive in an `IndexedStack`, so a one-time fetch in `initState`
/// would otherwise freeze on whatever was true the moment this tab was
/// first opened.
...
/// "July 2026 Adherence Report" — or, for a month still in progress (which
...
class CaregiverStorageScreen extends ConsumerWidget {
  ...
  Future<void> _onRefresh(WidgetRef ref) async {
    ...
    // Hold the pull-to-refresh spinner until the patient list is back, so
    // it doesn't snap shut before the new data has actually arrived.
...
/// Shows a patient header, a live "today" daily-adherence snapshot, a live
/// current-month adherence summary (matching everything the patient's own
/// downloadable PDF report contains — overall stats, time-of-day breakdown,
/// and the individual dose log) with a one-tap PDF download, and the list
/// of previously generated report PDFs.
...
class _PatientReportsSectionState
  ...
  ({String patientId, String patientName}) get _monthlyKey =>
          ...
          children: [
            ...
            dailyAsync.when(
              ...
              error: (err, stack) => Padding(
                ...
                child: Text(
                  'Unable to load today\'s adherence right now.',
                  style: GoogleFonts.inter(
      ...
      children: [
        ...
        if (report.totalDoses > 0) ...[
          ...
          Padding(
            ...
            child: Text(
              'No doses logged for this patient this month yet.',
              style:
