    ...
    return Scaffold(
      ...
      body: patientsAsync.when(
        ...
        data: (patients) {
          ...
          // No standing "Action Required" summary banner here anymore —
          // a delayed or missed dose now surfaces as the same one-time
          // pop-up dialog the patient sees on their own device (fired
          // from within each `PatientHomeCard` once a dose crosses the
          // 60-minute unresolved mark), rather than a persistent warning
          // strip that lingers even after doses are resolved.
          return ListView.separated(
            padding: const EdgeInsets.all(20),
            itemCount: patients.length,
            separatorBuilder: (context, index) => const SizedBox(height: 20),
            itemBuilder: (context, index) {
              return PatientHomeCard(patient: patients[index]);
            },
          );
        },
      ),
